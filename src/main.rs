...
#[tokio::main]
async fn main() {
    // println!("{}", serde_yaml::to_string(&DAG::crd()).unwrap());
    ...
    let crd_api: Api<DAG> = Api::all(kubernetes_client.clone());
    let context: Context<ContextData> = Context::new(ContextData::new(kubernetes_client.clone()));

    let _ = tokio::spawn(async move {
        let pods: Api<Pod> = Api::all(kubernetes_client.clone());
        let watcher = watcher(pods, ListParams::default());
        tokio::pin!(watcher);
        while let Some(m) = watcher.next().await {
            // panic and crash on error, app will restart
            match m.unwrap() {
...
async fn reconcile(echo: DAG, context: Context<ContextData>) -> Result<ReconcilerAction, Error> {
    ...
    return match determine_action(&echo) {
        Action::Create => {
            ...
            deploy::deploy(client, &echo.name(), &echo.spec, &namespace).await?;
            Ok(ReconcilerAction {
        ...
        Action::Delete => {
            ...
            deploy::delete(client.clone(), &echo.name(), &namespace).await?;
...
fn determine_action(echo: &DAG) -> Action {
