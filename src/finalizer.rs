...
pub async fn add(client: Client, name: &str, namespace: &str) -> Result<DAG, Error> {
    let api: Api<DAG> = Api::namespaced(client, namespace);
...
///
/// Note: Does not check for resource's existence for simplicity.
pub async fn delete(client: Client, name: &str, namespace: &str) -> Result<DAG, Error> {
    let api: Api<DAG> = Api::namespaced(client, namespace);
    let finalizer: Value = json!({
        "metadata": {
            "finalizers": null
